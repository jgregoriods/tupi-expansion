...
makeBiomeRasters2 <- function(type="null") {
    biomes <- vector(mode="list", length=7)
    for (i in 2:7) {
        bio <- raster(paste("rasters/S", i, "biomes_", i - 1, "k.asc", sep=""))
        proj4string(bio) <- wgs
        if (type == "null") {
            bio[values(bio) != 1 & values(bio) != 3 & values(bio) != 7] <- 0
            bio[values(bio) > 0] <- 1
        } else {
            bio[values(bio) != 1] <- 0
        }
        bio.m <- projectRaster(bio, ele.m)
        bio.m[values(bio.m) > 0] <- 1
        if (type != "null") {
            eco <- focal(bio.m, w=matrix(1,3,3), fun=mean, na.rm=T)
            bio.m[values(bio.m) == 1 & values(eco) < 1] <- 2
    ...
    for (i in 2:7) {
        writeRaster(biomes[[i]], paste("layers2/veg_", type, "/veg_", i - 1, "000.asc", sep=""))
    }
}
